...
public class StudentResultService {
    ...
    // FIX: Use a unique key per user or remove @Cacheable for dynamic user data
    @Cacheable(value = "student_performance", key = "#root.methodName + '_' + T(org.springframework.security.core.context.SecurityContextHolder).getContext().getAuthentication().getName()")
    ...
    @Transactional(readOnly = true)
    // FIX: Use a different cache name for admin data to avoid collisions
    @Cacheable(value = "admin_results")
